...
def train(args):
    ...
    for epoch in range(args.n_epochs):
        for i, X in enumerate(dset_loaders):
            ...
            d_fake = D(fake)
            print(d_real.shape)
            real_labels = var_or_cuda(torch.ones_like(d_real))
            fake_labels = var_or_cuda(torch.zeros_like(d_fake))
        ...
        # print('Iter-{}; , D_loss : {:.4}, G_loss : {:.4}, D_acu : {:.4}'.format(iteration, d_loss.data[0], g_loss.data[0], d_total_acu.data[0]))

        if (epoch + 1) % args.image_save_step == 0:

            samples = fake.cpu().data[:8].squeeze().numpy()
            # input_samples = X.cpu().data[:8].squeeze().numpy()
            ...
            # SavePloat_Voxels(input_samples, image_path, iteration)
